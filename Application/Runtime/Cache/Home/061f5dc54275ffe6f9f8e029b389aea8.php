...
<div class="container">
    <form action="<?php echo U('editData');?>" method="post">
        <label >Name</label>
        <input type="text" id="fname" name="name" placeholder="<?php echo ($data['name']); ?>">
        <input type="hidden" name='id' value="<?php echo ($data['id']); ?>">

        <label >Weight</label>
        <input type="text"  name="weight" placeholder="<?php echo ($data['weight']); ?>">
        ...
        <input type="checkbox"  name="status" <?php if($data['status'] == 1): ?>checked<?php endif; ?>>Display
        <input id='testNameHidden' type='hidden' value='No' name='status'>
        <br>
        <br>
